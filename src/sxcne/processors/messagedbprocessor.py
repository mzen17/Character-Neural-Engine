...
# ! THiS IS A WORK-IN-PROGRESS! Status: Not yet decided
...
def initialize():
    ...
    sql_command = "CREATE TABLE IF NOT EXISTS chat(chatid INTEGER PRIMARY KEY, user TEXT, output TEXT)"
...
def cleanup():
    ...
    sql_command = "DROP TABLE IF EXISTS chat"
    connection.execute(sql_command)
    connection.commit()
...
def push_chat(chatID: int, user: str, message: str, newEvent: Optional[str] = None):
